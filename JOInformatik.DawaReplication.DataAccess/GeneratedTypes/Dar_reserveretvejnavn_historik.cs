//-----------------------------------------------------------------------------------------------
// This is auto-generated code.
//-----------------------------------------------------------------------------------------------
// This code was generated by JO Informatik DAWA Replication tool version 1.0.2.0
// Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.
//-----------------------------------------------------------------------------------------------
using Microsoft.SqlServer.Types;
using Newtonsoft.Json;
using System;
using System.CodeDom.Compiler;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;

namespace JOInformatik.DawaReplication.DataAccess
{
    /// <summary>TODO.</summary>
    [GeneratedCode("JO Informatik DAWA Replication tool", "version 1.0.2.0")]
    [Table("dar_reserveretvejnavn_historik")]
    public partial class Dar_reserveretvejnavn_historik : ReplicationBase
    {
        /// <summary>This constructor calls OnCreated().</summary>
        public Dar_reserveretvejnavn_historik()
        {
            EntityUpdated = DateTime.Now;
            OnCreated();
        }

        /// <summary>
        /// DETTE ER PKEY. Unik ID for den angivne række. 
        /// </summary>
        [DatabaseGenerated(DatabaseGeneratedOption.None), Column("rowkey")]
        [Required()]
        public virtual int Rowkey { get; set; }

        /// <summary>
        /// Startidspunktet for rækkens virkningstid.
        /// </summary>
        
        [Column("virkningstart")]
        [Required()]
        public virtual DateTime Virkningstart { get; set; }

        /// <summary>
        /// Sluttidspunktet for rækkens virkningstid. 
        /// </summary>
        
        [Column("virkningslut")]
        public virtual DateTime? Virkningslut { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("id")]
        [Required()]
        public virtual Guid Id { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("status")]
        [Required()]
        public virtual int Status { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("navneområde", TypeName = "varchar(2000)")]
        public virtual string Navneområde { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("reservationudløbsdato")]
        [Required()]
        public virtual DateTime Reservationudløbsdato { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("reserveretafkommune", TypeName = "varchar(4)")]
        public virtual string Reserveretafkommune { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("reserveretnavn", TypeName = "varchar(200)")]
        public virtual string Reserveretnavn { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("reserveretudtaltnavn", TypeName = "varchar(200)")]
        public virtual string Reserveretudtaltnavn { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("reserveretvejadresseringsnavn", TypeName = "varchar(200)")]
        public virtual string Reserveretvejadresseringsnavn { get; set; }

        /// <summary>
        /// Ikke tilgængelig
        /// </summary>
        
        [Column("retskrivningskontrol", TypeName = "varchar(200)")]
        public virtual string Retskrivningskontrol { get; set; }

        /// <summary>Overwrite this method to do your own initialization of the entity.</summary>
        partial void OnCreated();

        /// Returns the primary key as a string (includes all columns).</summary>
        public override string DawaPkey
        {
            get
            {
                if (_dawaPkey == null)
                {
                    _dawaPkey = Rowkey.ToString();
                }
                return _dawaPkey;
            }
        }

        [Column("entity_updated")]
        [Required()]
        /// <summary>Tidspunktet da rækken blev oprettet eller ændret i databasen hos JOI.</summary>
        public DateTime EntityUpdated { get; set; }

    }
}
